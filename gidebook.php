...
<html lang="en">
  ...
  <link rel="stylesheet" href="./style.css" />
    ...
    <form action="gidebook_process.php" class="home-offers" method="post">
        <?php



        // SQL query to retrieve hotel information
        $sql = "SELECT * FROM tour_gide_info";
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<span class='offer-text'>" . $row['full_name'] . "</span>";
                echo "<span class='offer-text1'>" . $row['location'] . "</span>";
                echo "<span class='offer-text2'>" . $row['tour_offerings'] . "</span>";
                echo "<span class='offer-text2'>" . $row['cost_par_hour'] . "</span>";
                echo "<input type='checkbox' name='selected_topurgide[]' value='" . $row['id'] . "'> ";
                echo "</div>";
                echo "</div>";
